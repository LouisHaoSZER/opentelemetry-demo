...
#
# 构建 email 镜像并导出为 tar.gz, 用于离线 (无远程仓库) 分发到 TKE 节点。
#
# 用法:
#   IMAGE_TAG=2.0.4 ./build-and-export.sh
#
# 环境变量:
#   IMAGE_REPO   镜像仓库前缀 (不含 tag), 默认 otel-demo-email。
...
# email 的 Dockerfile 使用 repo-root 相对路径 (COPY ./src/email/...)
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"

GIT_SHA="$(git -C "${REPO_ROOT}" rev-parse --short HEAD 2>/dev/null || echo nogit)"

IMAGE_REPO="${IMAGE_REPO:-otel-demo-email}"
IMAGE_TAG="${IMAGE_TAG:-dev-${GIT_SHA}}"
PLATFORM="${PLATFORM:-linux/amd64}"
OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/_dist}"

FULL_IMAGE="${IMAGE_REPO}:${IMAGE_TAG}"
TAR_NAME="otel-demo-email_${IMAGE_TAG}.tar"
